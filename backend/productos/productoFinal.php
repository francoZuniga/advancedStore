...
if (!isset($_SESSION['cargarProducto']) || $_SESSION['cargarProducto'] != 2) {
...
$filasPublicacion = $publicacion->setPublicacionProductoId($_SESSION['idProducto']);

$idPublicacion = $publicacion->getIdPublicacion();

$filas = $producto->setProductoId($_SESSION['idProducto']);
$filasImagenes = $imagenes->setImagenPublicacionIdPublicacion($_SESSION['idPublicacion']);
        ...
        <div class="col">
          <div class="collapse table-bordered" id="multiCollapseExample1">
            <div class="card card-body">
              <?php
              ficha($_SESSION['tipoProducto']);
              function ficha($dato){

                    if($dato == "accesorios") {
                      $accesorios = new fichaAccesorios();
                      $filasAccesorios = $accesorios->getFicha($_SESSION['idProducto']);

                      if (empty($filasAccesorios)) {
                        echo "no hay registros";
                      }
                      else {
                        foreach($filasAccesorios as $fila){
                          echo "
                          <table class=\"table bg-light\">
                            <tbody>
                             <tr>
                      ...
                      if ($dato == "audio") {
                        $audio = new fichaAudio();
                        $filasAudio = $audio->getFicha($_SESSION['idProducto']);

                        if (empty($filasAudio)) {
                          echo "no hay registros";
                        }
                        else {
                          foreach($filasAudio as $fila) {
                            echo "
                            <table class=\"table bg-light\">
                              <tbody>
                               <tr>
                        ...
                        $variedad = new fichaVariedad();
                        $filasVariedad = $variedad->getFicha($_SESSION['idProducto']);

                        if (empty($filasVariedad)) {
                          echo "no hay regisrtos";
                        }
                        else {
                          foreach ($filasVariedad as $fila) {
                            echo "
                            <table class=\"table bg-light\">
                              <tbody>
                               <tr>
                               ...
                               <tr>
                                 <th scope=\"row\">Tipo Productos:</th>
                                 <td>".$fila['tipoProductos']."</td>
                               </tr>
                               <tr>
                                 <th scope=\"row\">Descripcion</th>
                                 <td>".$fila['descripcion']."</td>
